...
{
    public abstract class UTFPointerInputModule : BaseInputModule
    {
		[SerializeField]
		protected bool simulateMouseWithGamepad = false;
		protected bool _simulateMouseWithGamepad = false;

        [SerializeField]
        protected float mouseSpeed = 20f;
        [SerializeField]
        protected RectTransform virtualCursor;

		protected Vector2 virtualCursorPosition = new Vector2(860, 540);
		...
		protected Vector2 cursorPosition
		{
			...
			set
			{
				if (_simulateMouseWithGamepad)
				{
					...
					v.x = Mathf.Clamp(v.x, 0f, Screen.width);
					v.y = Mathf.Clamp(v.y, 0f, Screen.height);

					virtualCursorPosition = v;
				}
			}
		}
        ...
        protected override void Start()
        {
            ...
            // Simulate mouse with gamepad on consoles
            _simulateMouseWithGamepad = simulateMouseWithGamepad ||
                Application.platform == RuntimePlatform.PS4 ||
                Application.platform == RuntimePlatform.XboxOne;

            if (!_simulateMouseWithGamepad ) virtualCursor.gameObject.SetActive(false);
        }
